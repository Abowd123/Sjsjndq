...
# ═════════════════════════════════════════════════════════════════
#  PERSONAL-USE LOCK — only OWNER_ID may use this bot.
#  Any update from anyone else is dropped before it ever reaches a
#  handler. If OWNER_ID isn't set yet (fresh deploy, value <= 0),
#  updates pass through untouched so the very first /start can still
#  auto-claim ownership (see the existing auto-claim logic below).
# ═════════════════════════════════════════════════════════════════
_orig_process_new_updates = bot.process_new_updates


def _update_sender_id(u) -> Optional[int]:
...
def _personal_use_process_new_updates(updates):
    allowed = []
    for u in updates:
        uid = _update_sender_id(u)
        if OWNER_ID <= 0 or uid is None or uid == OWNER_ID:
            allowed.append(u)
            continue
        # politely reject non-owner senders, then drop the update
        try:
            if getattr(u, "message", None) is not None:
                bot.send_message(
                    u.message.chat.id,
                    f"{G['lock']} This bot is private and for personal use only.",
                )
...
bot.process_new_updates = _personal_use_process_new_updates
...
def get_or_create_user(u: types.User, ref: Optional[int] = None) -> Tuple[Dict[str, Any], bool]:
    ...
    if is_new:
        db["users"][key] = {
            ...
            "plan": "lifetime" if u.id == OWNER_ID else "free", "plan_expires": None,
        ...
        if u.id == OWNER_ID:
            db["users"][key]["plan"] = "lifetime"
            db["users"][key]["plan_expires"] = None
